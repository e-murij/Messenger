...
class Server(threading.Thread, metaclass=ServerMaker):
    ...
    def __init__(self):
        ...
        self.listen_address, self.listen_port = self.arg_parser(config['SETTINGS']['Default_port'],
                                                                config['SETTINGS']['Listen_Address'])
        self.transport = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.database = ServerStorage(os.path.join(config['SETTINGS']['Database_path'],
        ...
        while True:
            ...
            for mes in self.messages:
                try:
                    ...
                    self.SERVER_LOGGER.info(f'Связь с клиентом с именем {mes[DESTINATION]} была потеряна')
    ...
    def process_client_message(self, message, client):
        ...
        # Если клиент выходит
            ...
            self.SERVER_LOGGER.info(f'Клиент {message[ACCOUNT_NAME]} корректно отключился от сервера.')
...
def main():
    ...
    main_window.active_clients_table.setModel(gui_create_model(server.database))
    ...
    def show_statistics():
        ...
        stat_window.history_table.setModel(create_stat_model(server.database))
